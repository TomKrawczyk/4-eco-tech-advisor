...
Deno.serve(async (req) => {
  try {
    ...
    const file = new File([json], `raport_dane_${date}.json`, { type: 'application/json' });
    const up = await base44.integrations.Core.UploadFile({ file });
    return new Response(JSON.stringify({ file_url: up?.file_url || null }), { status: 200, headers: { 'Content-Type': 'application/json' } });
  } catch (e) {
