...
export default function PVCalculator() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const response = await base44.functions.invoke('generatePVCalculatorPDF', { zuzycie, orientacja, cenaPradu, result, weatherData });
      const { pdf_base64, filename } = response.data;
      openOrDownloadPDF(pdf_base64, filename || `kalkulator-pv-${new Date().toISOString().split('T')[0]}.pdf`);
