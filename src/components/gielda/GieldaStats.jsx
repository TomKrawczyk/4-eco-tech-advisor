...
import { Clock, AlertTriangle, CalendarClock, CalendarDays, CheckCircle2, PhoneCall, RotateCcw, FileText } from "lucide-react";
...
export default function GieldaStats({ pins, currentUserEmail, tab }) {
  ...
  const gorace = pins.filter((p) => p.source === "PhoneContact" && p.phone_status === "Kontakt do doradcy" && !p.isAssigned).length;
  const ponowne = pins.filter((p) => p.source === "PhoneContact" && p.phone_status === "Do ponownego kontaktu" && !p.isAssigned).length;
  const nowe = pins.filter((p) => p.source === "ContactLead" && !p.isAssigned).length;
  const sla = pins.filter((p) => !p.isAssigned && isSla(p)).length;
  return (
    <StatsGrid
      tiles={[
        { label: "Gorące (do doradcy)", value: gorace, icon: PhoneCall, color: "text-red-600 bg-red-50 border-red-200" },
        { label: "Do ponownego kontaktu", value: ponowne, icon: RotateCcw, color: "text-orange-600 bg-orange-50 border-orange-200" },
        { label: "Nowe leady", value: nowe, icon: FileText, color: "text-yellow-600 bg-yellow-50 border-yellow-200" },
