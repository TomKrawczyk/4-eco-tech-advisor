...
export default function ROICalculator() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const response = await base44.functions.invoke('generateROICalculatorPDF', { kosztInstalacji, rocznaProdukcja, cenaPradu, kosztUtrzymania, inflacjaEnergii, degradacjaPaneli, result });
      const { pdf_base64, filename } = response.data;
      openOrDownloadPDF(pdf_base64, filename || `analiza-roi-${new Date().toISOString().split('T')[0]}.pdf`);
